...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized && cognito.isRequestUser) {
    let updated = false;
    if (updates.comment) {
      const commenters = await database.query("SELECT * from users where cognito_id = $1 AND version = (SELECT MAX (version) FROM users where cognito_id = $1)", cognito_id);
      updated = await database.query("UPDATE service_requests SET comments = comments || $2::jsonb where id = $1", ticket_id, { ...updates.comment, "created_at": new Date(), cognito_id, name: `${commenters[0].first_name} ${commenters[0].last_name}` });
    } else {
      updated = await database.updateById("service_requests", ticket_id, updates);
    }
    if (updated) {
      const request = await database.query("SELECT sr.*, u.first_name as creator_first, u.last_name as creator_last, uu.first_name as assignee_first, uu.last_name as assignee_last from service_requests sr JOIN users u on u.cognito_id = sr.cognito_id LEFT JOIN users uu on uu.cognito_id = sr.assignee where sr.id = $1", ticket_id);
      return {
        statusCode: 200,
        headers: getHeaders(),
        body: JSON.stringify({
