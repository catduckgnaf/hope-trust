...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized || cognito.isRequestUser) {
    ...
    if (created) {
      const request = await database.query(`SELECT
      sr.*,
      ...
      where sr.id = $1
      AND u.status = 'active'
      AND uuu.status = 'active'`, created);
      const system = await database.query("SELECT u.first_name, u.last_name, u.email from system_settings ss JOIN users u on u.cognito_id = ss.customer_support where ss.id = 'system_settings' AND u.status = 'active'");
      const request_type = ticket.request_type === "other_request_type" ? "other" : ticket.request_type.replace(/_/g, " ");
      await sendTemplateEmail(system[0].email, {
        first_name: system[0].first_name,
        last_name: system[0].last_name,
        template_type: "new_ticket",
        merge_fields: {
          submitted: moment().format("MM/DD/YYYY [at] hh:mm A"),
          first_name: system[0].first_name,
          creator_first: request[0].creator_first,
          creator_last: request[0].creator_last,
          account_first: request[0].account_first,
          account_last: request[0].account_last,
          request_type: capitalize(request_type),
          type: request_type,
          title: request[0].title,
          body: request[0].body,
          subject: `New ${capitalize(request_type)} Ticket`,
          preheader: `A new ${request_type} ticket has been submitted by ${request[0].creator_first} ${request[0].creator_last}. Please review.`
        }
      });
      return {
        statusCode: 200,
