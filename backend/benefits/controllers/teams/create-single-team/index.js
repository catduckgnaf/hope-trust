...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized || cognito.isRequestUser) {
    let config_id;
    let id;
    delete newTeam.groups;
    let created_config = await database.query("SELECT * from benefits_config where cognito_id = $1", cognito_id);
    let existing_record = await database.query("SELECT * from teams where cognito_id = $1", cognito_id);
    ...
    await database.updateById("teams", id, { status: "active" });
    const teams = await database.query(`SELECT
          w.*,
          bc.signature_id,
