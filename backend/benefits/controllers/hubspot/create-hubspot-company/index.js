...
exports.handler = async (event) => {
  ...
  const existing_company = await database.query("SELECT * from referral_configurations where name = $1", name);
  if (!existing_company.length) {
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        "payload": {
          companyId: existing_company[0].hubspot_company_id,
          type: existing_company[0].type
        }
      })
    };
  }
