...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized || cognito.isRequestUser) {
    ...
    let existing_record = await database.query("SELECT * from wholesalers where cognito_id = $1", cognito_id);
    ...
    if (!existing_record.length) {
      delete newWholesaler.parent_id;
    ...
    await database.updateById("wholesalers", id, { status: "active" });
    const wholesalers = await database.query(`SELECT
          w.*,
          bc.signature_id,
    ...
    if (!wholesalers.length) {
      return {
        ...
        body: JSON.stringify({
          "success": false,
          "message": "Could not find wholesaler record."
        })
      };
    }
    return {
      ...
      body: JSON.stringify({
        "success": true,
        "message": "Successfully fetched wholesaler record.",
        "payload": wholesalers[0]
      })
    };
  } else {
    return {
