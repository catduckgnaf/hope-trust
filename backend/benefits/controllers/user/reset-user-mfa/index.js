...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized && cognito.isRequestUser) {
    ...
    if (!disabled) {
      return { statusCode: 400, headers: getHeaders(), body: JSON.stringify({ success: false, message: "Could not disable user MFA." }) };
    } else {
      await sendTemplateEmail(users[0].email, {
        ...
        merge_fields: {
          ...
          preheader: `${users[0].first_name}, your MFA has been disabled`
        }
      });
      return { statusCode: 200, headers: getHeaders(), body: JSON.stringify({ success: true, message: "Successfully disabled user MFA."}) };
    }
  } else {
