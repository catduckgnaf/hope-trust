...
const getCognitoUser = async (event, account_id) => {
  let providerId = event.requestContext.authorizer.claims ? event.requestContext.authorizer.claims.sub : false;
  if (providerId) {
    let claimed_pool_id = event.requestContext.authorizer.claims.iss.split("/").pop();
    ...
    if (cognitoUser) {
      ...
      if (account_id) user["isAuthorized"] = isAuthorized(event.resource, merged, account_id);
      return user;
    }
    return false;
  }
