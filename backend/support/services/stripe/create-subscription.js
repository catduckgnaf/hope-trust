...
const createSubscription = async (subscription_config, plan, customer) => {
  ...
  if ((plan.coupon && plan.coupon.valid && plan.coupon.duration !== "once")) charge.coupon = plan.coupon.id;
  if (subscription_config.coupon) charge.coupon = subscription_config.coupon;
  if ((subscription_config.discount_code && subscription_config.discount_code.valid && subscription_config.discount_code.duration !== "once")) charge.coupon = subscription_config.discount_code.id;
