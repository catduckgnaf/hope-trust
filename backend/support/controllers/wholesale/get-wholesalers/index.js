...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized) {
    const wholesalers = await database.query(`SELECT
      ...
      JOIN users u on u.cognito_id = bc.cognito_id AND u.version = (SELECT MAX (version) FROM users uu where uu.cognito_id = u.cognito_id AND uu.status = $1)
      where w.status = $1 AND u.status = $1`, "active");
