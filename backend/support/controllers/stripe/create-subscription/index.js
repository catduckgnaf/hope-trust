...
exports.handler = async (event) => {
  ...
  const { new_customer, customer_id, failure_plan, charge_config, subscription_config, type, charge_type, cognito_id, target_account_id } = JSON.parse(event.body);
  ...
  if (cognito.isAuthorized) {
    ...
    if (new_customer) {
      ...
      const customer_user = await database.queryOne("SELECT * from users where cognito_id = $1 AND status = $2", new_customer.cognito_id, "active");
      if (customer_user) await database.updateById("users", customer_user.id, { customer_id: created_customer.response.id });
    }
    switch (charge_type) {
      case "credits":
    ...
    if (subscription.success) {
      ...
      if (charge_type === "credits" && !charge_error) {
        let amount = 0;
        const partner = await database.queryOne("SELECT p.* from partners p JOIN users u on u.cognito_id = p.cognito_id where u.customer_id = $1 AND p.status = 'active' AND u.status = 'active'", customer_id);
      ...
      if (charge_type === "single-partner") await updateAccountMembershipPlanPermissions(plan, target_account_id, "create");
      return {
        statusCode: 200,
        headers: getHeaders(),
        body: JSON.stringify({
