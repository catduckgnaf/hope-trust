...
exports.handler = async (event) => {
  ...
  const { target_account } = JSON.parse(event.body);
  const cognito = await getCognitoUser(event, account_id);
  if (cognito.isAuthorized) {
    const user = await database.queryOne("SELECT * from users where cognito_id = $1 AND status = $2", target_account, "active");
    const partner = await database.queryOne("SELECT * from partners where cognito_id = $1 AND status = $2", target_account, "active");
    const referral = await database.queryOne("SELECT * from referral_configurations where name = $1", partner.name);
    const oldMembership = await database.queryOne("SELECT * from account_memberships where cognito_id = $1 AND account_id = $2 AND status = $3", target_account, target_account, "active");
    const oldMembershipUpdated = await database.updateById("account_memberships", oldMembership.id, { "status": "inactive" });
    const domain = user.email.split("@")[1];
    const is_domain_approved = (partner.domain_approved || WEBMAIL_PROVIDER_DOMAINS.includes(domain));
    if (oldMembershipUpdated) {
      ...
      if (referral) {
        ...
        if (is_domain_approved) await database.updateById("partners", partner.id, { "domain_approved": true });
        if (!WEBMAIL_PROVIDER_DOMAINS.includes(domain)) await database.updateById("referral_configurations", referral.id, { "status": "active", "domains": JSON.stringify([...referral.domains, domain]).replace("[", "{").replace("]", "}") });
        if (referral_code) {
      ...
      if (created) {
        return {
          ...
          body: JSON.stringify({
            ...
            "payload": { ...created, first_name: user.first_name, last_name: user.last_name, domain_approved: is_domain_approved }
          })
        };
      }
      return {
