const { database } = require("../../../postgres");
const { getHeaders, warm } = require("../../../utilities/request");

exports.handler = async (event) => {
  if (event.source === "serverless-plugin-warmup") return warm;
  const { org_name } = event.pathParameters;
  const entity = await database.queryOne("SELECT logo from partners where name = $1 AND is_entity = true  AND logo IS NOT NULL AND status = 'active' AND version = (SELECT MAX (version) FROM partners where name = $1 AND is_entity = true AND status = 'active')", org_name);
  let default_image = "data:image/png;base64,";
  const base64ContentArray = entity ? entity.logo.split(",") : default_image.split(",");
  const mime_type = base64ContentArray[0].match(/[^:\s*]\w+\/[\w-+\d.]+(?=[;| ])/)[0];
  const base64Data = base64ContentArray[1];
  return {
    statusCode: 200,
    headers: {
      ...getHeaders(),
      "Content-type": mime_type
    },
    body: base64Data.toString("base64"),
    isBase64Encoded: true
  };
};
