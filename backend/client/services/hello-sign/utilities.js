...
const setCustomFields = (templates, signers, partner, user, cost, additional_plan_credits, additional_plan_cost) => {
	let custom_fields = [];
	for (let i = 0; i < templates.length; i++) {
		...
		if (required_custom_fields[template].includes("Seats_Number") && !custom_fields.find((c) => c.name === "Seats_Number")) custom_fields.push({ name: "Seats_Number", value: (additional_plan_credits) || 0 });
