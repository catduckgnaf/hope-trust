...
const getSessionData = async (survey_id, survey_name, session_id, hidden_fields = ["HTTP", "SCRIPT"]) => {
  ...
  return fetch(url, {
    ...
    .then((res) => {
      console.info(`-----RESPONSE FOR SESSION ${session_id}-----`, survey_id, res.status, res.statusText);
      status = res.status;
      return res.json();
    })
    .then((data) => {
      ...
      let requests = [];
      let failed = [];
      let total_requests = Object.keys(groups).length;
      ...
      for (let group_id in groups) {
        ...
        }).catch((error) => console.error("SURVEY GIZMO REST API ERROR: ", error));
        requests.push(prom);
      }
      let res = Promise.all(requests).then((result) => {
        result.forEach((response) => {
          ...
          if (response && response.data) meta[response.data.id] = response.data;
        });
        if (failed.length) {
          console.error(`${failed.length} of ${total_requests} requests failed.`);
          console.table(failed, ["status", "survey_id", "group_id", "request_status", "statusText"]);
          return false;
        }
    ...
    .catch((error) => {
      console.error("SURVEY GIZMO SURVEY DATA ERROR: ", error);
      return error;
    });
};
