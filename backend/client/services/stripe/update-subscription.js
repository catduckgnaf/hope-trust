...
const updateSubscription = async (user, plan, initial_plan, external_coupon) => {
  ...
  if (isCustomer.success) {
    ...
    if (isSubscribed.success) {
      ...
      let canceledPlans = [];
      if (initial_plan) canceledPlans = await cancelPlanItems(isSubscribed.subscription_id, initial_plan.is_metered);
      charge.billing_cycle_anchor = "unchanged";
      if (!initial_plan.monthly) charge.billing_cycle_anchor = "now";
