...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized) {
    if (plaid_item_id) {
      ...
      if (item) {
        const remaining_accounts = await database.query("SELECT * from account_beneficiary_assets where plaid_item_id = $1 AND status = 'active'", plaid_item_id);
    ...
    const updated_record = await database.updateById("account_beneficiary_assets", id, { status: "inactive" });
    if (updated_record) {
      return {
        statusCode: 200,
        headers: getHeaders(),
        body: JSON.stringify({
          success: true,
          message: "Successfully deleted beneficiary asset record"
        })
      };
    }
    return {
