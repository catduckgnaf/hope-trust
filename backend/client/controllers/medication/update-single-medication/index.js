...
exports.handler = async (event) => {
  ...
  const { medication_id, account_id } = event.pathParameters;
  const { updates } = JSON.parse(event.body);
  const cognito = await getCognitoUser(event, account_id);
  if (cognito.isAuthorized) {
    const updated = await database.updateById("medications", medication_id, updates);
    if (updated) {
      const medication = await database.queryOne(`SELECT DISTINCT ON (m.id)
        ...
        WHERE m.id = $1`, updated.id);
      return {
        statusCode: 200,
        headers: getHeaders(),
        body: JSON.stringify({
          "success": true,
          "message": "Successfully updated medication record",
          "payload": medication
        })
      };
    }
    return {
      ...
      body: JSON.stringify({
        "success": false,
        "message": "Could not update medication record."
      })
    };
  }
  return {
