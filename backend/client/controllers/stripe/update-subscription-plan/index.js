...
const { capitalize, getAccountUsers } = require("../../../utilities/helpers");
...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized && cognito.isRequestUser) {
    ...
    const creator = users.find((user) => user.is_customer && !user.linked_account);
      ...
      if (updated.success) {
        ...
        if (oldAccountUpdated) {
          ...
          if (created) {
            ...
            if (subscription) {
              const archived = await database.updateById("subscriptions", subscription.id, { status: "inactive", cognito_id: cognito.id });
              if (archived) {
              ...
              return {
                ...
                body: JSON.stringify({
                  "success": false,
                  "message": "Could not update existing subscription record."
                })
              };
            }
            return {
              ...
              body: JSON.stringify({
                "success": false,
                "message": "Could not find existing subscription record."
              })
            };
          }
          return {
