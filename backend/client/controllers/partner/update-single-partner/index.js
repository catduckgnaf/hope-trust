...
exports.handler = async (event) => {
  ...
  if (cognito.isAuthorized && cognito.isRequestUser) {
    const partner = await database.queryOne("SELECT * from partners where cognito_id = $1 AND version = (SELECT MAX (version) FROM partners where cognito_id = $1)", cognito.id);
    if (partner) {
