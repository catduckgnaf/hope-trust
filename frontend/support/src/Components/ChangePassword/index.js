...
class ChangePassword extends Component {
  ...
  render() {
    ...
    const passwordCheck = checkPasswordConditions(16, new_password, confirm_password, old_password);
    return (
      <RowBody>
        <RowHeader>
          <Row>
        ...
        <RowContentSection span={12}>
          <SettingsButtonContainer span={12}>
            <ChangePasswordButton disabled={!(old_password && passwordCheck.pass)} onClick={() => this.updatePassword(old_password, new_password, confirm_password)} small nomargin primary green outline>{is_loading ? <FontAwesomeIcon icon={["far", "spinner"]} spin /> : "Change Password"}</ChangePasswordButton>
          </SettingsButtonContainer>
        </RowContentSection>
      </RowBody>
    );
