...
class CustomerSupportUserSignUpForm extends Component {
  ...
  checkEmail = (event) => {
    ...
    if (!this.debouncedFn) {
      this.debouncedFn = debounce(() => {
        ...
        if (email.includes("@")) {
          checkUserEmail(email, "customer-support");
        } else {
  ...
  render() {
    let { details, setNewState, claimThisEmail, missingFields, creator_email_error, creator_email_valid, is_checking_creator_email } = this.props;
    const { revealed, other_phone_error, home_phone_error, fax_error } = this.state;
    const passwordCheck = checkPasswordConditions(16, details.password, details.confirmPassword);
