...
const DetailsButton = (props) => {
  ...
  if (props.is_authorized) {
    ...
    if (buttons.length > 3) {
      return (
        <Button nomargin blue rounded outline small onClick={() => {
              ...
              return (
                <Fragment key={key}>
                  ...
                  loading_state[key] && button.onClick.async && !button.text
                    ...
                    : <span {...((button.tooltipProps && button.tooltipProps["data-tooltip-content"]) && button.tooltipProps)}><button.Component key={key} {...button.props} onClick={() => clickHandler(button.onClick, setLoading, rootDispatch, key, props.dispatch)}>{loading_state[key] && button.onClick.async ? <FontAwesomeIcon icon={["fal", "spinner"]} spin /> : button.text}</button.Component></span>
                  }
                </Fragment>
              );
            }
...
const clickHandler = async (onClick, setLoading, rootDispatch, key, dispatch) => {
  if (onClick.async) {
...
const tablePropsInit = ({ isSelectable = false, columnReordering = true, rowReordering = false, csvExport = true, ...props }) => {
  ...
  return {
    ...
    columns,
    paging: {
      enabled: props.paging && !props.virtualScrolling && props.has_data,
      pageIndex: 0,
...
const ReduxTableComponent = (props) => {
  ...
  const account = current_user.accounts.find((a) => a.account_id === session.account_id);
                ...
                <SearchContainerSection widget={props.widget ? 1 : 0} align="right" xs={6} sm={6} md={2} lg={2} xl={2}>
                  <SearchButtonWrapper>
                    {tableProps.searchText || tableProps.paging.pageIndex > 0 || (filterActive && filterValue.items.length) || !!initial_data.length
                      ? <TableButton small={props.widget ? 1 : 0} secondary blue outline rounded onClick={() => {
                        onFilterChanged(defaultFilter);
                        dispatch(search(""));
          ...
          <PaginationSection xs={12} sm={12} md={9} lg={9} xl={9} align="right">
              ...
              ? (
                <>
                  {props.newRow
                    ? (
                      <>
                              ...
                              <NewItemFieldWrapper>
                                <NewItemFieldWrapperPadding>
                                  <NewItemFieldWrapperInner>
                                    ...
                                    <TableButton small={props.widget ? 1 : 0} disabled={!checkFields(newItemConfig, store_data) || !props.newRow.fields.every((f) => f.condition(newItemConfig.values[f.name], newItemConfig))} nomargin marginleft={10} outline green onClick={() => saveItem(newItemConfig, updateNewItemField, rootDispatch)}>{newItemConfig.loading ? <FontAwesomeIcon icon={["fas", "spinner"]} spin /> : "Save"}</TableButton>
                                    <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline danger onClick={() => updateNewItemField({ ...newItemConfig, isCreatingNewItem: false, values: {} })}>Cancel</TableButton>
                                  </NewItemFieldWrapperInner>
                                </NewItemFieldWrapperPadding>
                              </NewItemFieldWrapper>
                            )
                        ...
                        {!newItemConfig.isCreatingNewItem
                          ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline blue onClick={() => (!props.newRow.isTableDispatch ? rootDispatch(props.newRow.onClick(...props.newRow.arguments)) : dispatch({ type: props.newRow.onClick, ...props.newRow.arguments }))}>{props.newRow.buttonText || "New"}</TableButton>
                          : null
                        }
                      </>
                    )
                  ...
                  {selectedData.length && props.deleteMultiple
                    ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline danger onClick={() => deleteMultiple(selectedData.map((s) => s.id), props.deleteMultiple)}>{`Delete ${selectedData.length} ${(selectedData.length === 1) ? "Row" : "Rows"}`}</TableButton>
                    : null
                  }
                </>
              )
            ...
            {!newItemConfig.isCreatingNewItem
              ? (
                <>
                  {exportPDFFunction
                    ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline blue onClick={() => rootDispatch(exportPDFFunction((filtered ? filtered : tableProps.data), tableProps.searchText))}>Export PDF</TableButton>
                    : null
                  }
                  {props.additionalButton && is_authorized
                    ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline blue onClick={() => rootDispatch(props.additionalButton.onClick(...props.additionalButton.arguments))}>{props.additionalButton.buttonText}</TableButton>
                    : null
                  }
                  {tableProps.paging.pageIndex >= 1
                    ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline blue onClick={() => dispatch(updatePageIndex(Number(tableProps.paging.pageIndex - 1)))}>Previous Page</TableButton>
                    : null
                  }
                  {((tableProps.paging.pageIndex + 1) < (tableProps.data.length / tableProps.paging.pageSize)) && tableProps.data.length && pages > 1
                    ? <TableButton small={props.widget ? 1 : 0} nomargin marginleft={10} outline blue onClick={() => dispatch(updatePageIndex(Number(tableProps.paging.pageIndex + 1)))}>Next Page</TableButton>
                    : null
                  }
                </>
              )
