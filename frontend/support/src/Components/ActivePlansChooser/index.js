...
const breakpoint = () => {
  if (isMobileOnly) return "300";
  if (isTablet) return "700";
  if (isDesktop) return "1200";
};

export const ActivePlansChooser = (props) => {
  const { page, type, stateRetriever, stateConsumer, transfer = false, benefits = false } = props;
  const plans_state = useSelector((state) => state.plans);
  let active_plans = plans_state[`active_${type}_plans`];
  const current_choice = stateRetriever("plan_choice");
  if (benefits) active_plans = active_plans.filter((p) => p.type === "benefits");
  else active_plans = active_plans.filter((p) => p.type === "user");
  let sorted_plans = orderBy(active_plans, [(plan) => (plan.name === "Free"), "monthly"], ["desc", "asc"]).map((p, index) => {
    return { ...p, index };
  });
  if (transfer || benefits) {
    sorted_plans = orderBy(active_plans.filter((p) => p.monthly), ["monthly"], ["asc"]).map((p, index) => {
      return { ...p, index };
    });
  }

  return (
    <ActivePlansMain width={breakpoint()}>
      <ActivePlansPadding>
        <ActivePlansInner>
                  ...
                  return (
                    <ActivePlan {...props}>
                      <ActivePlanPadding>
                        ...
                        <ActivePlanExpand onClick={() => stateConsumer(`plan_${plan_index}_expanded`, !stateRetriever(`plan_${plan_index}_expanded`))}>{stateRetriever(`plan_${plan_index}_expanded`) ? "Less" : "More"}</ActivePlanExpand>
                        <ActivePlanFooter>
                          <ActivePlanFooterPadding>
                            <ActivePlanFooterInner cost={plan.monthly}>
                              ...
                              <ActivePlanFooterAction span={4}>
                                <ActivePlanFooterActionButton type="button" onClick={!isActive ? () => stateConsumer("plan_choice", plan) : null}>
