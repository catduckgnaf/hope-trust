...
import { confirmSignUp, resendSignUp, cancelSignup } from "../../store/actions/customer-support-signup";
...
class ConfirmCustomerSupportSignUpForm extends Component {
  static propTypes = {
  ...
  resend = async () => {
    const { details, resendSignUp } = this.props;
    const { creatorDetails } = details;
    this.setState({ isLoading: true });
    setTimeout(async () => {
      await resendSignUp(creatorDetails.email);
      this.setState({ isLoading: false });
    }, 2000);
  };

  render() {
    const { details, setNewState, cancelSignup } = this.props;
    const { code, creatorDetails } = details;
        ...
        <ConfirmSignupFormMain>
          ...
          <Row gutter={20}>
            <Col xs={12} sm={12} md={4} lg={4} xl={4}>
              <ConfirmSignupButton disabled={!this.validateForm(code)} type="submit" secondary outline green>Verify</ConfirmSignupButton>
            </Col>
            <Col xs={12} sm={12} md={4} lg={4} xl={4}>
            ...
            <Col xs={12} sm={12} md={4} lg={4} xl={4}>
              <ConfirmSignupButton type="button" onClick={() => cancelSignup(1, creatorDetails.email)} primary outline blue>Cancel</ConfirmSignupButton>
            </Col>
          </Row>
        </ConfirmSignupFormMain>
      </FormContainer>
...
export default connect(mapStateToProps, dispatchToProps)(ConfirmCustomerSupportSignUpForm);