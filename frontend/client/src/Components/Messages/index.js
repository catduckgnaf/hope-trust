...
class MessageSettings extends Component {
  ...
  render() {
    const { message, user, copyToClipboard, navigateTo } = this.props;
        ...
        <MessagesMain>
          <MessagesPadding>
            <EmailContainer>
              ...
              <Button noshadow nomargin marginleft={5} marginright={user.username ? 1 : 5} blue small onClick={() => copyToClipboard(`${user.username || user.cognito_id}${email}`, "Email Address")}>Copy</Button>
              {!user.username
                ? <Button noshadow nomargin small green onClick={() => navigateTo("/settings", "?tab=profile")}>Set Username</Button>
                : null
              }
            </EmailContainer>
            <MessagesInner>
              <GenericTable
                ...
                fields={[
                  ...
                  },
                  {
                    ...
                    type: "select",
                    options: [
                      { caption: "Incoming", value: "incoming" },
                      { caption: "Outgoing", value: "outgoing" }
                    ]
                  },
                  {
                    caption: "Created",
