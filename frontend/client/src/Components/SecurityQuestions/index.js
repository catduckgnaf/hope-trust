...
import { createSecurityQuestionResponse, getSecurityQuestions } from "../../store/actions/security-questions";
import {
  InputWrapper,
  SelectWrapper,
  InputLabel,
...
async componentDidMount() {
  const { security, getSecurityQuestions } = this.props;
  ...
  setupSecurityQuestions = async () => {
    const { showNotification, location, security } = this.props;
          ...
          <RowContentSection span={12}>
            <SettingsButtonContainer paddingleft={1} span={12}>
              {securityQuestionInfo.security_question_answer_1 && securityQuestionInfo.security_question_answer_2 && securityQuestionInfo.security_question_answer_3
                ? <Button onClick={() => this.saveSecurityQuestions()} nomargin small green>{is_loading ? <FontAwesomeIcon icon={["far", "spinner"]} spin /> : "Save Security Questions"}</Button>
                : null
              }
            </SettingsButtonContainer>
          </RowContentSection>
