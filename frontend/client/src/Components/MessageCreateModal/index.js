...
import { Button, RequiredStar, InputLabel, InputWrapper, Input, TextArea, SelectStyles, InputHint } from "../../global-components";
import { capitalize, verifyEmailFormat } from "../../utilities";
...
class MessageCreateModal extends Component {
  ...
  constructor(props) {
    super(props);
    const { defaults = {}, relationship, accounts, user } = props;
    const target_users = accounts.map((account) => {
      const option_items = relationship.list.map((u) => {
  ...
  sendMessage = async () => {
    ...
    if (!verifyEmailFormat(to_email)) {
      showNotification("error", "Required Fields", "This email is not a valid format.");
      return false;
    }
  ...
  render() {
    ...
    const { email_error, from_email, to_email, to_first, to_last, subject, body, sending_message, target_users, current_user } = this.state;
           ...
           {(current_user && viewing) || (!viewing && target_users.length)
                  ...
                  <MessageModalContentSection span={12}>
                    <InputWrapper>
                      ...
                      <Input
                        ...
                        onChange={this.setField}
                        readOnly={viewing}
                        valid={verifyEmailFormat(to_email || current_user?.email)}
                      />
                      <InputHint error>{email_error}</InputHint>
                    </InputWrapper>
                  </MessageModalContentSection>
                </>
              )
            ...
            <MessageModalFooter span={12}>
              {updating || !viewing
                ? (
                  <>
                    <Button disabled={sending_message} green onClick={() => this.sendMessage()}>{sending_message ? <FontAwesomeIcon icon={["fad", "spinner"]} spin /> : "Send"}</Button>
                    <Button danger onClick={() => closeMessage()}>Cancel</Button>
                  </>
                )
                : <Button danger onClick={() => closeMessage()}>Close</Button>
              }
            </MessageModalFooter>
          </MessageModalContent>
        </MessageModalMain>
