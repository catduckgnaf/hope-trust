...
class BillingMetrics extends Component {
  ...
  constructor(props) {
    super(props);
    const { account, session, accounts } = props;
    ...
    const current_plan = current_account.user_plan;
    ...
    const paid_accounts = (account.subscriptions.active && account.subscriptions.active.length) ? account.subscriptions.active.filter((s) => s.type === "user") : [];
    ...
    const credits_used = paid_accounts.reduce((a, b) => a + b.account_value, 0);
    const amount = (credits_used || 0);
    ...
    if (current_discount && ["repeating", "forever"].includes(current_discount.duration)) {
      ...
      if (current_discount.amount_off) discounted = (current_discount.amount_off/100);
    }
    const totalWithDiscount = (amount - discounted);
    this.state = {
      metrics: [
        {
          title: "Tier",
        ...
        },
        {
          ...
          tooltip: <Tooltip placement={isMobileOnly ? "left" : "top"} background={lighten(0.6, theme.hopeTrustBlue)} radius={6} padding={0} border="none" color={theme.hopeTrustBlue} content={<div style={{ width: "200px", boxShadow: "0 4px 8px rgba(0,0,0,0.2)", padding: "15px", borderRadius: "6px", background: lighten(0.6, theme.hopeTrustBlue) }}>Your first month of Hope Trust is covered in your first payment. Your subscription will start in {moment(trial_end).fromNow(true)} days</div>}><FontAwesomeIcon size="sm" icon={["fad", "question-circle"]} /></Tooltip>
        },
        {
          title: "Next Payment",
          value: next_payment,
          show: (status === "active" || account.isFetchingCustomer) && credits_used,
...
export default connect(mapStateToProps, dispatchToProps)(BillingMetrics);
