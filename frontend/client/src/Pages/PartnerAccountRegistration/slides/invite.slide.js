...
const config = {
  ...
  form: [
    {
      ...
      required: false,
      value: () => retrieveReduxState("user").partner_data.name,
      disableIf: () => true
    },
    {
  ...
  lifecycle: {
    ...
    shouldRender: (stateRetriever) => !stateRetriever("authorized") && !retrieveReduxState("partners").organization_partners.length && (!retrieveReduxState("partners").isFetchingOrgPartners && retrieveReduxState("partners").requestedOrgPartners)
  }
};
