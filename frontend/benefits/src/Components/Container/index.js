...
class Container extends Component {
  ...
  render() {
    const { children, title, xs, sm, md, lg, xl, id, padding, paddingtop, height, action, overflow, viewall, bottom, transparent } = this.props;
            ...
            <ContainerInner transparent={transparent} height={height} overflow={overflow}>
              <ContainerHeader bottom={bottom}>
                ...
                <ContainerSection xs={!hasBoth ? 4 : 8} sm={!hasBoth ? 4 : 8} md={6} lg={6} xl={6} align="right">
                  {action
                    ? <Button onClick={() => action.func()} small green outline rounded>{action.title}</Button>
                    : null
                  }
                  {viewall
                    ? <Button onClick={() => viewall.func()} small outline green rounded marginright={10}>{viewall.title}</Button>
                    : null
                  }
                </ContainerSection>
              </ContainerHeader>
