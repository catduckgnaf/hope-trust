...
import { updateBenefitsConfig } from "../../store/actions/account";
...
import { closePartnerLogoModal } from "../../store/actions/session";
...
class PartnerLogoModal extends Component {
  ...
  saveAvatar = async (logo) => {
    const { updateBenefitsConfig } = this.props;
    this.setState({ saving: true });
    await updateBenefitsConfig({ logo });
        ...
        <PartnerLogoModalMain>
          <PartnerLogoModalMainPadding>
            <PartnerLogoModalMainInner>
              ...
              <PartnerLogoModalMainInnerSection span={12}>
                <PartnerLogoModalMessage>As an organization entity, it is your responsibility to set up your organization for co-branding. To get this started, please upload a logo for {user.benefits_data.name}. This logo will be co-branded across any paid client accounts managed by your organization.<br /><br /><HeavyFont>For best results, choose a square image of at least 512x512 pixels.</HeavyFont></PartnerLogoModalMessage>
              </PartnerLogoModalMainInnerSection>
              <PartnerLogoModalMainInnerSection align="center" span={12}>
                {editing_logo
                  ...
                  : <ReactAvatar src={user.benefits_data.logo || imageSrc} size={250} name={user.benefits_data.name} round />
                }
                <LogoButtonContainer>
                  <Button rounded blue outline nomargin onClick={() => this.setState({ editing_logo: true })}>Browse</Button>
                  {imageSrc || user.benefits_data.logo
                    ? <Button disabled={saving} marginleft={10} rounded green outline nomargin onClick={() => closePartnerLogoModal()}>{saving ? <FontAwesomeIcon icon={["fad", "spinner"]} spin /> : "Finish"}</Button>
                    : null
                  }
                </LogoButtonContainer>
              </PartnerLogoModalMainInnerSection>
...
const dispatchToProps = (dispatch) => ({
  ...
  updateBenefitsConfig: (updates) => dispatch(updateBenefitsConfig(updates))
});
