          ...
          <Col span={12}>
            <Row>
              <IdleMainButtonContainer span={12}>
                <Button type="button" onClick={() => this.stayLoggedIn()} green outline rounded>Stay Logged In</Button>
                <Button type="button" onClick={() => logOut()} danger outline rounded>Logout</Button>
              </IdleMainButtonContainer>
            </Row>
          </Col>
        </Row>
...
const dispatchToProps = (dispatch) => ({
  refreshUser: () => dispatch(authenticated.refreshUser()),
  updateUserStatus: (cognito, status, idle) => dispatch(authenticated.updateUserStatus(cognito, status, idle)),
