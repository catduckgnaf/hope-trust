        ...
        LogRocket.track(`Cognito user confirmed. - ${user.first_name} ${user.last_name} - ${user.email}`);
				...
				if (newUser.success) {
					...
					const newAccount = await API.post(
						...
						`/accounts/create/${loggedIn.username}`,
						{
							...
							body: {
                "account_name": `${createdUser.first_name} ${createdUser.last_name}`,
                ...
                "cognito_id": createdUser.cognito_id,
					...
					if (newAccount.success) {
						...
						const storedUser = await API.get(apiGateway.NAME, `/users/${loggedIn.username}`, { headers: { Authorization: token } });
...
export const clearCustomerSupportSignupState = () => async (dispatch) => {
  dispatch({ type: CLEAR_CUSTOMER_SUPPORT_SIGNUP_STATE });
};
