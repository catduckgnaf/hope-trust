...
export const dispatchRequest = (ticket) => async (dispatch) => {
  const account = store.getState().user && store.getState().user.accounts ? store.getState().user.accounts.find((account) => account.account_id === store.getState().session.account_id) : {};
